  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground">Synchronizing Intelligence...</p>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="p-20 text-center">
        <p className="text-red-500 font-black uppercase tracking-widest text-xs">Synchronization Error</p>
        <p className="text-muted-foreground text-[10px] mt-2 tracking-widest uppercase">Failed to retrieve growth signals.</p>
      </div>
    );
  }
    ...
    <div className="space-y-16 animate-reveal">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 pb-10 border-b border-border">
        <div className="space-y-4">
          <Link href={postId ? `/post/${post?.slug}` : '/dashboard'} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:text-foreground transition-colors">
            <ArrowLeft size={14} /> {postId ? 'Back to Narrative' : 'Back to Dashboard'}
          </Link>
          <h1 className="text-5xl md:text-7xl font-black tracking-tighter uppercase leading-none">
            {isOverall ? 'Global Reach' : 'Narrative Growth'}
          </h1>
          {!isOverall && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          { label: 'Appreciation', value: totals.likes, icon: Heart, color: 'text-rose-500' },
          { label: 'Dialogues', value: totals.comments, icon: MessageSquare, color: 'text-blue-500' },
          { 
            label: 'Avg Attention', 
          ...
          <div key={i} className="group p-10 rounded-[3rem] bg-white border border-border hover:border-foreground transition-all flex flex-col justify-between h-56 shadow-sm hover:shadow-2xl hover:shadow-foreground/5">
      ...
      {/* Main Growth Graph */}
      <div className="p-10 md:p-16 rounded-[4rem] border border-border bg-white space-y-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-2">
            ...
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em]">Synchronization volume mapped over temporal dimensions</p>
          </div>
            ...
            daily.slice(typeof window !== 'undefined' && window.innerWidth < 768 ? -30 : -60).map((day: any, i: number) => {
              ...
              return (
                <div key={i} className="flex-1 group relative h-full flex flex-col justify-end">
                  <div 
                    className="w-full bg-muted/10 hover:bg-black transition-all rounded-t-xl md:rounded-t-2xl min-h-[4px]" 
                    style={{ height: `${Math.max(height, 2)}%` }}
                  >
                    <div className="absolute bottom-full mb-4 left-1/2 -translate-x-1/2 hidden group-hover:block z-20">
                      <div className="bg-black text-white p-4 rounded-2xl shadow-2xl space-y-2 min-w-[140px]">
        ...
        <div className="flex justify-between pt-8 border-t border-border/50 text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em]">
           <span>T-Minus 60 Days</span>
           <span className="opacity-40">Temporal Archive</span>
           <span>Universal Now</span>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="p-10 rounded-[3rem] bg-muted/5 border border-border space-y-8">
            <h3 className="text-xs font-black uppercase tracking-[0.3em] flex items-center gap-2">
              <MessageSquare size={16} /> Resonance Breakdown
            </h3>
            <div className="space-y-6">
               <div className="flex justify-between items-end border-b border-border pb-4">
                 <span className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Public Claps</span>
                 <span className="text-3xl font-black tracking-tighter">{totals.likes}</span>
               </div>
               <div className="flex justify-between items-end border-b border-border pb-4">
                 <span className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Community Discourses</span>
                 <span className="text-3xl font-black tracking-tighter">{totals.comments}</span>
               </div>
               <div className="flex justify-between items-end border-b border-border pb-4">
                 <span className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Total Residence Time</span>
          ...
          <div className="p-10 rounded-[3rem] bg-black text-white space-y-8 shadow-2xl shadow-foreground/20">
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-2">
                 <p className="text-[10px] font-medium leading-relaxed italic text-white/40">
                   "Your data is the echo of your narrative's resonance in the digital void."
                 </p>
              </div>
              <Link href="/dashboard" className="block">
                <Button className="w-full bg-white text-black font-black uppercase tracking-widest text-[10px] h-14 rounded-2xl hover:bg-white/90">
                  Global Dashboard
                </Button>
              </Link>
            </div>
